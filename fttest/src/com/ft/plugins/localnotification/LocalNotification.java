package com.ft.plugins.localnotification;
...
public class LocalNotification extends Plugin {
	...
	public PluginResult execute(String action, JSONArray optionsArr,
		...
		if (action.equalsIgnoreCase("add")) {
			...
			final String url = alarmOptions.getUrl();

			persistAlarm(alarmId, optionsArr);
			return this.add(daily, url, title, subTitle, ticker, alarmId,
					alarmOptions.getCal());
	...
	public PluginResult cancelAllNotifications() {
		 ...
		 * by one.
		 */
		final SharedPreferences alarmSettings = this.ctx.getSharedPreferences(
	...
	private boolean persistAlarm(String alarmId, JSONArray optionsArr) {
		final Editor alarmSettingsEditor = this.ctx.getSharedPreferences(
